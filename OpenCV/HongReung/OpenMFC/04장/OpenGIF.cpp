// OpenGIF.cpp: implementation of the COpenGIF class.
//
//////////////////////////////////////////////////////////////////////
...
long COpenGIF::sq(UBYTE i,UBYTE j)
{
  return((i-j)*(i-j));
}

void COpenGIF::ReadImageHeader(FILE *fp)
{
  int tnum, i, flag;
...
void COpenGIF::TheEnd()
{
  throw 0;
}

UBYTE COpenGIF::Xgetc(FILE *fin)
{
  int i;
  if ( ( i = fgetc(fin) ) == EOF ) {
    TheEnd(); 
...
void COpenGIF::GifDecode(FILE *fp, UBYTE *pix, GifImageHdr gifimage)
...
UBYTE *COpenGIF::GifSendData(UBYTE *pix, int bytes, UBYTE source[])
...
void COpenGIF::GifEncode(FILE *fout, UBYTE *pixels, int depth, int siz)
{
...
void COpenGIF::ClearTree(int cc, GifTree *root)
{
  int i;
  GifTree *newNode, **xx;
...
char *COpenGIF::AddCodeToBuffer(int code, short n, char *buf)
{
  int    mask;
...
IplImage *COpenGIF::loadGIF (const char *name)
{
...
if ((pix = (UBYTE *)malloc(temp * sizeof(UBYTE)) ) == NULL )
  return NULL;
try {
...
int COpenGIF::saveGIF (const char *name, IplImage *img)
{
