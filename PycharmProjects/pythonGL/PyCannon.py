...
def init():
    ...
    radangle = (float(angle) * 3.1415926) / 180.0
    # Solve for horizontal and vertical initial velocities
    horizvel = float(muzzvel) * cos(radangle)
    vertvel = float(muzzvel) * sin(radangle)
    # Print out the initial velocities in the console
    print("")
    print("Horizontal Velocity (m/s) = %f"%horizvel)
    print("Vertical Velocity (m/s) = %f"%vertvel)
...
def plottrajectory():
    ...
    print()
    print("Distance traveled (m) = %f"%dist)
    print("Maximum altitude (m) = %f"%maxheight)
...
def main():
    ...
    glutInit(sys.argv)
    glutCreateWindow(b"How Far Will It Go?")
